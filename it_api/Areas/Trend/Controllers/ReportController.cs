    ...
    public class ReportController : BaseController
    {
            ...
            else
            {
                ...
                if (list.Count > 0)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        if (targetModel.id == 11)
                        {
                            sheet.Range["G" + start_r].NumberFormat = "#,##0.0#";
                        }
        ...
        public async Task<JsonResult> xuattrend(LimitModel LimitModel, List<int> list_point, int timestamp, int type_bc, DateTime? date_from_prev, DateTime? date_to_prev)
        {
            ...
            if (LimitModel.object_id == 2)
            {
                ...
                var report = "";
                if (list_point.Contains(1680))//chứa điểm Cấp D-KN
                {
                    report = "PHÒNG SẠCH/ THIẾT BỊ CỦA CẤP SẠCH D PHÒNG KIỂM NGHIỆM VI SINH";
                }
                else if (list_point.Contains(1734))
                {
                    report = "PHÒNG SẠCH/ THIẾT BỊ CỦA CẤP SẠCH C PHÒNG KIỂM NGHIỆM VI SINH";
                }

                else if (list_point.Contains(971))
                {
                    report = "PHÒNG SẠCH/ THIẾT BỊ CỦA KHU VỰC NGHIÊN CỨU PHÁT TRIỂN";
                }

                else if (list_point.Contains(1382))
                {
                    report = "PHÒNG SẠCH/ THIẾT BỊ CỦA XƯỞNG DƯỢC LIỆU";
                }
                else if (list_point.Contains(2383))
                {
                    report = "PHÒNG SẠCH/ THIẾT BỊ CỦA XƯỞNG THUỐC NON-BETALACTAM";
                }
                else if (list_point.Contains(904))
                {
                    report = "PHÒNG SẠCH/ THIẾT BỊ CỦA TỔNG KHO";
                }
                else if (list_point.Contains(2285))
                {
                    report = "PHÒNG SẠCH/ THIẾT BỊ CỦA XƯỞNG DÙNG NGOÀI";
                }
